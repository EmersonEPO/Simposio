<?php
    ...
    $nick = strtoupper($nome);
    if(($dao->verificarUser($nick)) == true){
         echo"<script language='javascript'> 
                window.location.href='../presentation/index.php?erro=0'
             </script>";
    }else{
        //verifica se email ja existe no sistema!
        if(($dao->verificarEmail($email)) == true){
             echo"<script language='javascript'> 
                    window.location.href='../presentation/index.php?erro=1'
                 </script>"; 
        }else{
